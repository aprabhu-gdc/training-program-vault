"""Egnyte source sync adapter for downloading raw source files."""
...
class EgnyteSourceSyncAdapter:
    ...
    @staticmethod
    def parse_webhook_payload(payload: Any) -> list[SourceFileEvent]:
        ...
        for item in candidates:
            ...
            if not path:
                continue
            event_type = str(item.get("event_type") or item.get("action") or item.get("event") or item.get("type") or "updated")
            events.append(
                SourceFileEvent(
                    path=str(path),
                    event_type=event_type,
                    modified_at=(str(item.get("modified_at")) if item.get("modified_at") is not None else None),
    ...
    def is_in_scope(self, event: SourceFileEvent) -> bool:
        ...
        return self._settings.egnyte_training_folder_name in normalized and normalized.startswith("/" + self._settings.egnyte_sync_root.strip("/"))
    ...
    def download_file(self, path: str) -> Path:
        ...
        LOGGER.info("Downloading Egnyte file path=%s destination=%s", path, destination)
                ...
                for file_info in payload.get("files", []) or []:
                    ...
                    files.append(
                        SourceFileEvent(
                            ...
                            modified_at=str(file_info.get("last_modified") or file_info.get("lastModified") or file_info.get("uploaded") or ""),
